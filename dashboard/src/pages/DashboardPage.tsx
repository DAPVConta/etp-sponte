                      ...
                      <table className="w-full border-collapse text-xs tabular-nums">
                        ...
                        <tfoot>
                          <tr className="border-t-2 border-border/60 bg-muted/30 font-bold">
                            <td className="text-left px-2 py-2 sticky left-0 bg-muted/30 text-foreground uppercase tracking-wide text-[0.65rem]">Total</td>
                            {unidades.map(u => {
                              const totalPlan = grupos.reduce((s, g) => s + (dadosPorUnidade[u.id]?.[g.grupo]?.plan ?? 0), 0);

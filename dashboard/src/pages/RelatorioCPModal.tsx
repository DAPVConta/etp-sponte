...
interface LinhaGrupo {
  ...
  despesasDiretas: LinhaDespesa[];
  total: number;
}

export default function RelatorioCPModal({ unidade, mes, accentColor, onClose }: Props) {
  const [lancamentos, setLancamentos] = useState<LancamentoCP[]>([]);
    ...
    (async () => {
      ...
      try {
        const [lanc, pc] = await Promise.all([
          ContasPagarAPI.listarLancamentos({ unidadeIds: [unidade.id], mes }),
  ...
  // Agrupamento: grupo > subgrupo > despesa > lançamentos
  const arvore: LinhaGrupo[] = useMemo(() => {
    if (!plano.length) {
      // Sem plano de contas — agrupa tudo em um pseudo-grupo "Sem classificação" por categoria
      const porCat = new Map<string, LancamentoCP[]>();
      for (const l of lancamentos) {
        const k = l.categoria || '(Sem categoria)';
        if (!porCat.has(k)) porCat.set(k, []);
        porCat.get(k)!.push(l);
      }
      const despesas: LinhaDespesa[] = [...porCat.entries()].map(([nome, lanc]) => ({
        nome, lancamentos: lanc, total: lanc.reduce((s, x) => s + x.valorPago, 0),
      }));
      despesas.sort((a, b) => a.nome.localeCompare(b.nome));
      return [{
        nome: 'Sem plano de contas',
        subgrupos: [],
        despesasDiretas: despesas,
        total: despesas.reduce((s, d) => s + d.total, 0),
      }];
    }

    // Mapa categoria normalizada -> { grupo, subgrupo }
    const despesasPlano = plano.filter(p => p.tipo === 'despesa');
    const classif = new Map<string, { grupo: string; subgrupo: string | null; nome: string }>();
    for (const d of despesasPlano) {
    ...
    // Ordem dos grupos/subgrupos conforme plano
    ...
    const despOrdem = new Map<string, string[]>(); // key: `${grupo}::${sub||''}`
    for (const d of despesasPlano.sort((a, b) => a.sortOrder - b.sortOrder)) {
      const k = `${d.grupoNome || ''}::${d.subGrupoNome || ''}`;
      if (!despOrdem.has(k)) despOrdem.set(k, []);
      despOrdem.get(k)!.push(d.nome);
    }

    // Agregação
    // grupo -> subgrupo (ou '') -> despesa -> lançamentos
    const agg = new Map<string, Map<string, Map<string, LancamentoCP[]>>>();
    const categoriasSemClassif = new Map<string, LancamentoCP[]>();
    ...
    for (const g of todosGrupos) {
      ...
      for (const s of subNames) {
        const byS = byG.get(s);
        if (!byS) continue;
        const despNames = Array.from(new Set([...(despOrdem.get(`${g}::${s}`) || []), ...byS.keys()]));
        const despesas: LinhaDespesa[] = [];
        for (const d of despNames) {
          const lanc = byS.get(d);
          if (!lanc || lanc.length === 0) continue;
          despesas.push({ nome: d, lancamentos: lanc, total: lanc.reduce((sum, x) => sum + x.valorPago, 0) });
        }
        if (despesas.length) subgrupos.push({ nome: s, despesas, total: despesas.reduce((sum, d) => sum + d.total, 0) });
      }

      // Despesas diretas (sem sub-grupo)
      const bySemSub = byG.get('');
      const despesasDiretas: LinhaDespesa[] = [];
      if (bySemSub) {
        const despNames = Array.from(new Set([...(despOrdem.get(`${g}::`) || []), ...bySemSub.keys()]));
        for (const d of despNames) {
          const lanc = bySemSub.get(d);
          if (!lanc || lanc.length === 0) continue;
          despesasDiretas.push({ nome: d, lancamentos: lanc, total: lanc.reduce((sum, x) => sum + x.valorPago, 0) });
        }
      }

      const total = subgrupos.reduce((s, x) => s + x.total, 0) + despesasDiretas.reduce((s, x) => s + x.total, 0);
      if (total > 0 || subgrupos.length || despesasDiretas.length) {
        arv.push({ nome: g, subgrupos, despesasDiretas, total });
      }
    }

    // Sem classificação
    if (categoriasSemClassif.size) {
      const despesas: LinhaDespesa[] = [...categoriasSemClassif.entries()]
        .map(([nome, lanc]) => ({ nome, lancamentos: lanc, total: lanc.reduce((s, x) => s + x.valorPago, 0) }))
      ...
      >
        <div className="print-area bg-white w-[210mm] min-h-[297mm] my-6 mx-4 shadow-2xl p-10 relative">
          ...
          {!loading && !erro && arvore.length === 0 && (
            <p className="text-sm text-slate-500 text-center py-10">Nenhum lançamento pago no período.</p>
          )}
              ...
              <table className="w-full border-collapse text-[0.78rem]">
                <thead>
                  <tr className="bg-slate-100 border-b-2 border-slate-300">
                    <th className="text-left px-3 py-2 font-bold uppercase tracking-wider text-[0.65rem] text-slate-600">Grupo / Sub-grupo / Despesa</th>
                    <th className="text-right px-3 py-2 font-bold uppercase tracking-wider text-[0.65rem] text-slate-600 whitespace-nowrap w-[120px]">Valor Pago</th>
                  </tr>
                </thead>
                <tbody>
                  {arvore.map(g => (
...
// ── Bloco de grupo (incluindo sub-grupos e despesas diretas) ────────────────
function GrupoBloco({ grupo, accentColor, totalGeral }: { grupo: LinhaGrupo; accentColor: string; totalGeral: number }) {
  ...
  return (
    <>
      ...
      {/* Despesas diretas */}
      {grupo.despesasDiretas.map(d => (
        <DespesaLinha key={d.nome} despesa={d} indent="pl-8" />
      ))}
    </>
  );
}
...
function SubgrupoBloco({ subgrupo }: { subgrupo: LinhaSubgrupo }) {
  return (
    <>
      ...
      {subgrupo.despesas.map(d => (
        <DespesaLinha key={d.nome} despesa={d} indent="pl-10" />
      ))}
    </>
  );
}

function DespesaLinha({ despesa, indent }: { despesa: LinhaDespesa; indent: string }) {
      ...
      >
        <td className={`${indent} pr-3 py-1.5 text-slate-600`}>
          <span className="inline-flex items-center gap-1.5">
            ...
            {despesa.nome}
            <span className="text-[0.62rem] text-slate-400 ml-1">({despesa.lancamentos.length})</span>
          </span>
        </td>
        <td className="px-3 py-1.5 text-right tabular-nums text-slate-600">{fmtBRL(despesa.total)}</td>
      </tr>

      {/* Detalhamento opcional dos lançamentos */}
      {aberto && despesa.lancamentos.map((l, i) => (
        <tr key={`${l.contaPagarId}-${l.numeroParcela}-${i}`} className="bg-slate-50/40">
          ...
          <td className="px-3 py-1 text-right tabular-nums text-[0.7rem] text-slate-500">{fmtBRL(l.valorPago)}</td>
        </tr>
      ))}
    </>
  );

...
export default function LancamentoCPPage({ unidades, activeUnidade, accentColor }: Props) {
      ...
      <Card className="px-5 py-3 mb-6">
        <div className="flex flex-wrap items-center gap-x-4 gap-y-2">
          ...
          <div className="ml-auto relative">
            ...
            <Input
              placeholder="Pesquisar sacado/categoria..."
              value={search}
      ...
      <Card className="overflow-hidden">
        <div className="px-6 py-4 border-b border-border/50 flex items-center justify-between flex-wrap gap-3">
          <h2 className="text-base font-bold flex items-center gap-2">
            <Receipt size={17} style={{ color: accentColor }} />
            Lançamentos
              ...
              {filtrados.map(l => {
                ...
                return (
                  <TableRow key={`${l.contaPagarId}-${l.numeroParcela}-${l.unidadeId}`} className="hover:bg-slate-50/40 transition-colors">
                    ...
                    <TableCell className="py-2">
                      <SituacaoBadge situacao={l.situacaoParcela} />
                    </TableCell>
                  </TableRow>
                );
              })}
...
function SituacaoBadge({ situacao }: { situacao: string }) {
  ...
  const isPago     = s.includes('pag') || s.includes('quit');
  const isPendente = s.includes('pend');
  ...
  return (
    <span className={cn(
      ...
      isPago     && 'bg-emerald-50 text-emerald-700 border-emerald-200',
      isPendente && 'bg-amber-50 text-amber-700 border-amber-200',
      isCancel   && 'bg-slate-100 text-slate-500 border-slate-200',
      !isPago && !isPendente && !isCancel && 'bg-slate-50 text-slate-600 border-slate-200'
    )}>
      {situacao || '—'}
    </span>
  );
